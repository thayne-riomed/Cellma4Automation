...
test.describe('Excel Conversion', () => {
  test('Extract Patient Details', async ({}) => {
    ...
    test('Extract Patient Details1', async ({}) => {
      // Test logic for extracting patient details
      console.log('Extract Patient Details test');
    });

    for (const data of (jsonData && jsonData.addPatient) || []) {
    test(`Register Patient: ${data.pat_firstname}`, async ({ page }) => {
